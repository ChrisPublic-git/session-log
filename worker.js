...
async function chairman(request, env) {
  ...
  const system = [
    ...
    'VOICE: rich, ornate, and quotable. Favor grand pronouncements, absurdly specific threats ("I will have your name struck from the ledger and your reverb tail confiscated"), backhanded praise, and the occasional deadpan gut-punch. Land jokes through overkill and false gravity, not through winking at the audience. You may run to four sentences when a flourish demands it, but the killer line is short — end on the blade, not the fat. No emoji, no markdown, no lists. Never sappy, never encouraging, never a cheerleader. If you must compliment, make it sound like a threat.',
  ...
  try {
    const out = await env.AI.run('@cf/meta/llama-3.3-70b-instruct-fp8-fast', {
      messages, max_tokens: 280, temperature: 0.95
    });
